...
# Serve Vue frontend dist. If dist is missing, redirect to Vite dev server at localhost:3000.
...
if not _frontend_available:
    ...
    async def redirect_to_frontend():
        dev_url = config_manager.get("server.frontend_dev_url", "http://127.0.0.1:3000")
...
async def list_tasks():
    rows = await db.fetch_all(
        "SELECT task_id, user_request, status, created_at, updated_at FROM tasks ORDER BY created_at DESC"
    )
    return [
        {
            "task_id": r[0],
...
async def get_messages(
    task_id: str, layer: str = "L1", since: Optional[str] = None, limit: int = 100
):
